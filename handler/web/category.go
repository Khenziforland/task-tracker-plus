...
func (ct *categoryWeb) CategoryAddProcess(ctx *gin.Context) {
	var email string
	if temp, ok := ctx.Get("email"); ok {
		if contextData, ok := temp.(string); ok {
			email = contextData
	...
	session, err := ct.sessionService.GetSessionByEmail(email)
	if err != nil {
		ctx.Redirect(http.StatusSeeOther, "/client/modal?status=error&message="+err.Error())
		return
	}

	category_name := ctx.Request.FormValue("category_name")
	

	status, err := ct.categoryClient.AddCategory(session.Token, category_name)
	if err != nil {
		ctx.Redirect(http.StatusSeeOther, "/client/modal?status=error&message="+err.Error())
		return
	}
